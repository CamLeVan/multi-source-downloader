        ...
        try {
            ...
            for (int pieceId = startPieceId; pieceId <= endPieceId; pieceId++) {
                if (!state.isPieceCompleted(pieceId)) {
                    // Tuần 4: Trigger on-demand download (blocking)
                    System.out.println("VirtualFS: Piece " + pieceId + " not available, downloading on-demand...");
                    ...
                    if (state == null || !state.isPieceCompleted(pieceId)) {
                        System.err.println("VirtualFS: Failed to download piece " + pieceId + " on-demand");
