...
public class VirtualDownloaderFS extends FuseStubFS {
     ...
     */
    public VirtualDownloaderFS(String fileName, ManifestModel manifest, Scheduler scheduler,
                               PieceStorage pieceStorage, DownloadState downloadState, String localFilePath) {
    ...
    public int read(String path, Pointer buf, @size_t long size, @off_t long offset, FuseFileInfo fi) {
        ...
        try {
            // *** ĐÃ SỬA LỖI ***
            // Xóa logic đọc `stateStorage.loadState` (đọc từ đĩa)
            if (this.downloadState == null) {
                // Trạng thái (in-memory) chưa được khởi tạo
                return -EIO();
            }

            // Đọc trạng thái (in-memory)
            boolean needsDownload = IntStream.rangeClosed(startIndex, endIndex)
                    .anyMatch(i -> !this.downloadState.isPieceCompleted(i));
            ...
            if (needsDownload) {
                ...
                // Đây là mấu chốt: Chặn (block) luồng FUSE cho đến khi tải xong
                downloadFuture.get(); 
                
                System.out.println("VFS: On-demand download complete for pieces " + startIndex + "-" + endIndex);
            }

            // Sau khi chờ, dữ liệu chắc chắn đã có sẵn trên đĩa.
            ...
            e.printStackTrace();
            return -EIO(); // Trả về lỗi I/O cho ứng dụng đang đọc (VLC)
        }
    }
}