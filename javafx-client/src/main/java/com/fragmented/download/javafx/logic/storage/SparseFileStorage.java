...
/**
 * Handles the creation of large, sparse files and writing pieces to them in a thread-safe manner.
 */
public class SparseFileStorage implements PieceStorage {

    /**
     * Creates a sparse file of a given size. This pre-allocates the file size on the filesystem
     ...
     * This method is thread-safe. It uses a FileChannel to acquire an exclusive lock
     * only on the region of the file being written to, allowing other threads to write
     ...
     * @throws IOException if an I/O error occurs, or if the lock cannot be acquired.
     */
    @Override
        ...
        try (RandomAccessFile file = new RandomAccessFile(path, "rw");
            ...
            try {
                ...
                if (lock == null) {
                    ...
                    throw new IOException("Could not acquire exclusive lock on file region. Offset: " + offset + ", Size: " + data.length);
                }
