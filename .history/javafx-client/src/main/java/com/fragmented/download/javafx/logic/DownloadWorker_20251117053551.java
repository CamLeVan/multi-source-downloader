...
public class DownloadWorker {
     ...
     */
    private void downloadInternal(PieceModel piece, Consumer<byte[]> successCallback, Consumer<PieceModel> retryCallback, int sourceIndex) {
                ...
                try {
                    ...
                    if (!calculatedHash.equalsIgnoreCase(piece.getSha256())) {
                        ...
                        System.err.println("Hash mismatch for piece " + piece.getId() + 
                                " from source index " + sourceIndex + 
                                ". Expected: " + piece.getSha256() + ", Got: " + calculatedHash);
                        
                        // Check if there are more sources to try
                        if (sourceIndex + 1 < piece.getSources().size() && retryCallback != null) {
                            System.out.println("Retrying piece " + piece.getId() + " from next source (index " + (sourceIndex + 1) + ")");
                            
                            // Create a new piece with sources starting from the next index
                            List<String> remainingSources = new ArrayList<>(piece.getSources().subList(sourceIndex + 1, piece.getSources().size()));
