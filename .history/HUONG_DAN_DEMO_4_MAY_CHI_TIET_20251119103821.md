...
**Ghi lại IP:** `192.168.1.XXX` (ví dụ: `192.168.1.101`)
...
**Ghi lại IP:** `192.168.1.XXX` (ví dụ: `192.168.1.102`)
...
**Ghi lại IP:** `192.168.1.XXX` (ví dụ: `192.168.1.103`)
...
**Từ Máy A (Windows):**
```cmd
ping 192.168.1.102  # Máy B (Tracker)
ping 192.168.1.103  # Máy C (Client 1)
ping 192.168.1.104  # Máy D (Client 2)
```

**Từ Máy B (Ubuntu VM):**
```bash
ping -c 4 192.168.1.101  # Máy A (Origin)
ping -c 4 192.168.1.103  # Máy C (Client 1)
ping -c 4 192.168.1.104  # Máy D (Client 2)
```

**Từ Máy C (macOS):**
```bash
ping -c 4 192.168.1.101  # Máy A (Origin)
ping -c 4 192.168.1.102  # Máy B (Tracker)
ping -c 4 192.168.1.104  # Máy D (Client 2)
```
...
### 🔹 Máy B - Ubuntu VM (Tracker)

**Trong VM:**
```bash
sudo ufw allow 8081/tcp
sudo ufw allow 6881/tcp  # Nếu cần
sudo ufw status
```
...
**Kiểm tra config:**
```cmd
type ..\origin-server\src\main\resources\config.properties
```

**Kết quả mong đợi:**
```properties
server.host=192.168.1.101  # IP của Máy A
server.port=8443
```

---

### 🔹 Máy B - Ubuntu VM (P2P Tracker)
...
**Kết quả mong đợi:**
```properties
server.host=192.168.1.102  # IP của Máy B
server.port=8081
```
...
- Client Name: `Mac-Client-1` (hoặc tên bạn muốn)
...
- Tracker IP: `192.168.1.102` (Máy B)
- Origin IP: `192.168.1.101` (Máy A)
- Client Name: `Ubuntu-Client-2` (hoặc tên bạn muốn)
...
**Mở PowerShell:**
```cmd
cd D:\VKU_learning\HK5\NetworkPrograming\BigProject\theEnd\multi-source-downloader\origin-server
mvn spring-boot:run
```

**✅ Logs mong đợi:**
```
Creating a dummy 2GB file for serving: D:\...\origin-server\server_files\2GB.zip
...
**✅ Logs mong đợi:**
```
[INFO] Client Name: Mac-Client-1
[INFO] Local IP Address: 192.168.1.103
[INFO] Tracker URL: http://192.168.1.102:8081
[INFO] Origin Server URL: https://192.168.1.101:8443
```

**✅ UI sẽ hiển thị:**
- Window title: "Multi-Source Downloader - Mac-Client-1 (192.168.1.103)"
...
**✅ Logs mong đợi:**
```
[INFO] Client Name: Ubuntu-Client-2
...
**✅ UI sẽ hiển thị:**
- Window title: "Multi-Source Downloader - Ubuntu-Client-2 (192.168.1.104)"
- File list dropdown có `2GB.zip`
...
**✅ Kết quả mong đợi:**
- Download bắt đầu từ Origin Server (192.168.1.101)
...
**✅ Logs mong đợi trên Máy D:**
```
[TRACKER] GET /peers | FROM: 192.168.1.104 → TO: 192.168.1.102:8081
[TRACKER] ✓ Retrieved 1 peers | Peers: [192.168.1.103:6881]
[PEER] GET /piece/2GB.zip/5 | FROM: 192.168.1.104 → TO: 192.168.1.103:6881
[DOWNLOAD] ✓ Piece 5 completed | FROM: 192.168.1.103
```

**✅ Logs mong đợi trên Máy C:**
```
[PEER] GET /piece/2GB.zip/5 | FROM: 192.168.1.104 → TO: Peer (192.168.1.103:6881)
[PEER] ✓ Piece 5 sent | FROM: 192.168.1.103 → TO: 192.168.1.104
```
...
```bash
# Test Origin Server
curl -k https://192.168.1.101:8443/manifest/2GB.zip | head -20

# Test Tracker
curl http://192.168.1.102:8081/tracker/peers?fileId=test

# Test ping
ping -c 4 192.168.1.101  # Origin
ping -c 4 192.168.1.102  # Tracker
ping -c 4 192.168.1.104  # Client 2
```

**Từ Máy D (Ubuntu VM), test:**
```bash
# Test Origin Server
curl -k https://192.168.1.101:8443/manifest/2GB.zip | head -20
...
ping -c 4 192.168.1.101  # Origin
ping -c 4 192.168.1.102  # Tracker
ping -c 4 192.168.1.103  # Client 1
```
...
Tomcat started on port(s): 8080 (http)
[ORIGIN] GET /manifest/2GB.zip | FROM: 192.168.1.103 → TO: Origin:8080
[ORIGIN] GET /files/2GB.zip | Range: bytes=0-1048575 | FROM: 192.168.1.103
```
...
[INFO] Client Name: Mac-Client-1
[INFO] Local IP Address: 192.168.1.103
[STEP 01] Fetching manifest | FROM: 192.168.1.103 → TO: 192.168.1.101:8080
[TRACKER] POST /announce | FROM: 192.168.1.103 → TO: 192.168.1.102:8081
[DOWNLOAD] ✓ Piece 0 completed | FROM: 192.168.1.101
```
...
[INFO] Client Name: Ubuntu-Client-2
[INFO] Local IP Address: 192.168.1.104
[STEP 01] Fetching manifest | FROM: 192.168.1.104 → TO: 192.168.1.101:8080
[TRACKER] GET /peers | FROM: 192.168.1.104 → TO: 192.168.1.102:8081
[TRACKER] ✓ Retrieved 1 peers | Peers: [192.168.1.103:6881]
[PEER] GET /piece/2GB.zip/5 | FROM: 192.168.1.104 → TO: 192.168.1.103:6881
[DOWNLOAD] ✓ Piece 5 completed | FROM: 192.168.1.103
...
|-----|-----|------|------|-------------|
| Windows Host | 192.168.1.101 | Origin Server | 8080 | `origin-server/.../config.properties` |
| Ubuntu VM (Win) | 192.168.1.102 | P2P Tracker | 8081 | `p2p-tracker/.../config.properties` |
| macOS Host | 192.168.1.103 | Client 1 | 6881 | `javafx-client/.../config.properties` |
| Ubuntu VM (Mac) | 192.168.1.104 | Client 2 | 6881 | `javafx-client/.../config.properties` |
