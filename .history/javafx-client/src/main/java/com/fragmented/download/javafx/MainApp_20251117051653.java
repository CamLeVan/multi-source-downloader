...
public class MainApp extends Application {
    ...
    public void startDownload(String manifestUrl) {
        try {
            ...
            // 6. Register the new download with background services (VFS and P2P)
            // This assumes VFS and PeerServer have been refactored to handle multiple files.
            if (vfs != null) {
                // vfs.addFile(fileId, manifest, scheduler, pieceStorage);
            }
            // peerServer.addFile(fileId, pieceStorage, stateStorage);
    ...
    private void initializeBackgroundServices() {
        if (vfs == null) {
            new Thread(() -> {
                try {
                    // 1. Initialize PeerServer (independent of any single file)
                    // this.peerServer = new PeerServer(port); // Refactored constructor
                    // this.peerServer.start();
                    System.out.println("PeerServer started.");

                    // 2. Initialize VirtualDownloaderFS (independent of any single file)
                    // this.vfs = new VirtualDownloaderFS(); // Refactored constructor
                    ...
                    if (os.contains("win")) {
                        System.out.println("Running on Windows, VFS (Dokan) support is pending implementation.");
                        // Dokan mount logic would go here
                    } else if (os.contains("mac") || os.contains("nix") || os.contains("nux")) {
                        System.out.println("Running on macOS/Linux, attempting to mount with FUSE.");
                        // this.vfs.mount(mountPoint, true, false);
                        System.out.println("VFS mounted successfully at " + mountPoint);
                    } else {
                        System.out.println("Virtual File System is not supported on this OS: " + os);
                    }
                } catch (Exception e) {
