...
public class PeerServer implements AutoCloseable {
    ...
    private final ExecutorService serverExecutor = Executors
            .newSingleThreadExecutor(r -> new Thread(r, "peer-server-thread"));

    // Flag để track server đã sẵn sàng chưa
    private volatile boolean isReady = false;
    private final Object readyLock = new Object();

    public PeerServer(IStateStorage stateStorage, PieceStorage pieceStorage, ManifestModel manifest, String fileId,
            String localFilePath) throws IOException {
        // Improvement: Port conflict handling - try multiple ports if DEFAULT_PORT is
        // busy
        ...
        this.server.createContext("/piece",
                new PieceHandler(stateStorage, pieceStorage, manifest, fileId, localFilePath));
