...
public class MainApp extends Application {
    ...
    public void startDownload(String manifestUrl) {
        try {
            ...
            DownloadState inMemoryState = scheduler.getDownloadState(); 

            // 6. Create and start the PeerServer (ĐÃ SỬA LỖI 1)
            if (peerServer == null) {
                try {
            ...
            // 7. Mount the Virtual Filesystem (ĐÃ SỬA LỖI 2)
    ...
    /**
     * (Không thay đổi, code của bạn đã đúng)
     */
    private void mountVFS(String fileId, ManifestModel manifest, Scheduler scheduler, PieceStorage pieceStorage, DownloadState inMemoryState, String localFilePath) {
        if (vfs == null) {
            new Thread(() -> {
                try {
                    // Lời gọi constructor này (của bạn) đã đúng
