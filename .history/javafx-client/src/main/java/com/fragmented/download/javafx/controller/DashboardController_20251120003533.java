...
public class DashboardController {
    ...
    // OkHttpClient với timeout tăng lên để xử lý network chậm hoặc routing issues
    private final OkHttpClient httpClient = new OkHttpClient.Builder()
            .connectTimeout(30, TimeUnit.SECONDS)  // Tăng connect timeout từ 10s lên 30s
            .readTimeout(30, TimeUnit.SECONDS)      // Tăng read timeout từ 10s lên 30s
            .writeTimeout(30, TimeUnit.SECONDS)      // Tăng write timeout từ 10s lên 30s
    ...
    private void fetchFilesWithMetadata() {
        new Thread(() -> {
            ...
            int maxRetries = 3;
            long initialDelayMs = 1000; // 1 giây
            Exception lastException = null;
            
            for (int attempt = 0; attempt < maxRetries; attempt++) {
                try {
                    if (attempt > 0) {
                        // Wait trước khi retry (exponential backoff: 1s, 2s, 4s)
                        long delayMs = initialDelayMs * (long) Math.pow(2, attempt - 1);
                        System.out.println("[DEBUG] Retrying in " + delayMs + "ms (attempt " + (attempt + 1) + "/" + maxRetries + ")");
                        javafx.application.Platform.runLater(() -> {
                            statusLabel.setText("Retrying connection... (attempt " + (attempt + 1) + "/" + maxRetries + ")");
                        });
                        Thread.sleep(delayMs);
                    }
                    
                    System.out.println("[DEBUG] Fetching files from: " + infoUrl + " (attempt " + (attempt + 1) + "/" + maxRetries + ")");
                    ...
                    lastException = e;
                    System.err.println("[ERROR] Connection timeout (attempt " + (attempt + 1) + "/" + maxRetries + "): " + e.getMessage());
                    ...
                    lastException = e;
                    System.err.println("[ERROR] Connection refused (attempt " + (attempt + 1) + "/" + maxRetries + "): " + e.getMessage());
                    if (attempt == maxRetries - 1) {
                        // Last attempt failed
                        break;
                    }
                    ...
                    lastException = e;
                    System.err.println("[ERROR] Metadata API failed (attempt " + (attempt + 1) + "/" + maxRetries + "): " + e.getMessage());
                    if (attempt == maxRetries - 1) {
                        // Last attempt failed, fallback to legacy API
                        break;
                    }
                }
            }
            
            // Tất cả retries đều fail - hiển thị error message rõ ràng
            String errorMessage = "Cannot connect to Origin Server";
            if (lastException instanceof java.net.SocketTimeoutException) {
                errorMessage = "Connection timeout: Origin Server at " + originUrl + " is not responding. " +
                              "Please check:\n" +
                              "1. Origin Server is running\n" +
                              "2. Network connectivity (ping " + extractIPFromUrl(originUrl) + ")\n" +
                              "3. Firewall settings\n" +
                              "4. Correct IP address in config.properties";
            } else if (lastException instanceof java.net.ConnectException) {
                errorMessage = "Connection refused: Cannot reach Origin Server at " + originUrl + ". " +
                              "Please check:\n" +
                              "1. Origin Server is running on port " + extractPortFromUrl(originUrl) + "\n" +
                              "2. Network routing (client: " + extractIPFromUrl(originUrl) + ")\n" +
                              "3. Firewall allows connections";
            } else if (lastException != null) {
                errorMessage = "Failed to fetch file list: " + lastException.getMessage();
            }
            
            System.err.println("[ERROR] All retries failed. " + errorMessage);
            javafx.application.Platform.runLater(() -> {
    ...
    private String extractPortFromUrl(String url) {
        try {
            ...
            return url.startsWith("https") ? "8443" : "8080";
        } catch (Exception e) {
            return "unknown";
        }
    }
